...
async def ensure_contract_for_purchase(
    ...
    if subscription_id is not None:
        row = (await db.execute(
            select(Contract).join(
                ContractItem, ContractItem.contract_id == Contract.id)
            .where(ContractItem.subscription_id == subscription_id,
    ...
    contract = Contract(
        ...
        status="active", currency=currency or (cust.currency if cust else None),
        billing_interval="monthly", starts_at=utc_now(),
    )
    db.add(contract)
    await db.flush()
    await log_audit(db, entity="contract", entity_id=str(contract.id),
...
async def ensure_contract_item_for_purchase(
    db, *, contract: Contract, product_id: int,
    price_plan_id: int | None = None, subscription_id: int | None = None,
    quantity: int = 1, unit_amount: float | None = None,
    ...
    if row:
        return row
    item = ContractItem(
        contract_id=contract.id, product_id=product_id,
        price_plan_id=price_plan_id, subscription_id=subscription_id,
        description=description, quantity=quantity or 1,
        unit_amount=unit_amount,
    )
    db.add(item)
    await db.flush()
    await log_audit(db, entity="contract_item", entity_id=str(item.id),
...
async def resolve_product_for_invoice(db, invoice: Invoice):
    ...
    if invoice.subscription_id:
        ...
        if sub is not None:
            ...
            plan = await db.get(PricePlan, sub.price_plan_id) if sub.price_plan_id else None
...
async def _adopt_mail_item(db, invoice: Invoice):
    ...
    job = (await db.execute(
        select(MailProvisioningJob).where(
            MailProvisioningJob.invoice_id == invoice.id)
...
async def ensure_fulfillment_for_item(
    db, *, item: ContractItem, contract: Contract, invoice: Invoice,
    product=None, actor_type: str = "system", actor_id: str | None = None,
) -> Fulfillment:
    ...
    f = Fulfillment(
        org_id=contract.org_id, customer_id=contract.customer_id,
        contract_id=contract.id, contract_item_id=item.id,
        product_id=item.product_id, invoice_id=invoice.id,
        subscription_id=item.subscription_id,
        strategy=strategy, handler_key=handler,
    ...
    db.add(f)
    await db.flush()
    await log_audit(db, entity="fulfillment", entity_id=str(f.id),
                    action="fulfillment_created",
                    actor_type=actor_type, actor_id=actor_id,
                    org_id=contract.org_id,
                    payload={"contract_item_id": item.id, "handler": handler,
                             "strategy": strategy, "invoice_id": invoice.id})
    await db.flush()
    return f


async def _apply_result(db, f: Fulfillment, result, *, actor_type: str,
    ...
    if result.ok:
        ...
        await log_audit(db, entity="fulfillment", entity_id=str(f.id),
                        ...
                        actor_type=actor_type, actor_id=actor_id, org_id=org)
        ...
        await _notify_staff(db, f, "Provisionamento falhou",
        ...
        await log_audit(db, entity="fulfillment", entity_id=str(f.id),
                        ...
                        actor_type=actor_type, actor_id=actor_id, org_id=org)
        await _notify_staff(db, f, "Provisionamento precisa de revisão",
                            f"{f.handler_key} #{f.id}: {f.last_error or ''}"[:300])
    await db.flush()
...
async def _notify_staff(db, f: Fulfillment, title: str, body: str) -> None:
    try:
        ...
        for user in users:
            repo.add(Notification(user_id=user.id, customer_user_id=None,
...
async def run_fulfillment(db, fulfillment_id: int, *, actor_type: str = "system",
                          actor_id: str | None = None) -> Fulfillment | None:
    """Executa o handler e aplica o resultado (idempotente por estado)."""
    f = await db.get(Fulfillment, fulfillment_id)
    if not f or f.status in (FulfillmentStatus.ACTIVE.value,
    ...
    await log_audit(db, entity="fulfillment", entity_id=str(f.id),
                    ...
                    actor_type=actor_type, actor_id=actor_id, org_id=f.org_id)
    await db.flush()
    ctx = ProvisionContext(
        db=db, fulfillment=f, invoice=invoice, subscription=sub,
        product=product, price_plan=plan, customer=customer,
        org_id=f.org_id, actor_type=actor_type, actor_id=actor_id)
    try:
        ...
        result = ProvisionResult(ok=False, retryable=True,
...
async def after_payment(db, invoice_id: int, *, actor_type: str = "system",
                        actor_id: str | None = None) -> list[Fulfillment]:
    ...
    if product is None:
        logger.info("fulfillment: invoice %s sem produto rastreável; nada a fazer",
                    invoice_id)
        return []
    contract = await ensure_contract_for_purchase(
        db, customer_id=invoice.customer_id,
        org_id=await invoice_org(db, invoice),
        subscription_id=sub.id if sub else None,
        currency=invoice.currency, actor_type=actor_type, actor_id=actor_id)
    ...
    if adopted_item is not None:
        ...
        item = await ensure_contract_item_for_purchase(
            db, contract=contract, product_id=product.id,
            price_plan_id=plan.id if plan else None,
            subscription_id=sub.id if sub else None,
            quantity=1,
            unit_amount=float(plan.amount) if plan and plan.amount is not None else None,
            description=(plan.name if plan else product.name),
            actor_type=actor_type, actor_id=actor_id)
    f = await ensure_fulfillment_for_item(
        db, item=item, contract=contract, invoice=invoice, product=product,
        actor_type=actor_type, actor_id=actor_id)
...
async def process_due_fulfillments(db) -> dict:
    ...
    rows = (await db.execute(
        ...
        .order_by(Fulfillment.id).limit(50)
    )).scalars().all()
    done, failed = 0, 0
    for f in rows:
        try:
...
async def retry_fulfillment(db, fulfillment_id: int, *, actor_type: str,
                            actor_id: str | None) -> Fulfillment | None:
    """Retry operacional: re-enfileira (nunca executa provider no browser)."""
    f = await db.get(Fulfillment, fulfillment_id)
    if not f:
        return None
    if f.status not in (FulfillmentStatus.FAILED.value,
...
async def resolve_fulfillment(db, fulfillment_id: int, *, note: str | None,
...
async def cancel_fulfillment(db, fulfillment_id: int, *, note: str | None,
                             actor_type: str, actor_id: str | None):
    f = await db.get(Fulfillment, fulfillment_id)
...
async def sync_service_event(db, *, contract_item_id: int | None,
                             service_id: int | None, suspended: bool,
    ...
    q = select(Fulfillment).where(
        Fulfillment.status.in_([FulfillmentStatus.ACTIVE.value,
    ...
    n = 0
    for f in rows:
        target = (FulfillmentStatus.SUSPENDED.value if suspended
        ...
        await log_audit(db, entity="fulfillment", entity_id=str(f.id),
                        action=("fulfillment_suspended" if suspended
                                else "fulfillment_reactivated"),
                        actor_type=actor_type, actor_id=actor_id, org_id=f.org_id)
        n += 1
    if n:
        await db.flush()
