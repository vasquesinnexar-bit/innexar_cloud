...
class MailProvisioner:
    ...
    async def provision(self, ctx: ProvisionContext) -> ProvisionResult:
        ...
        if inv_id is None:
            return ProvisionResult(ok=False, waiting_input="invoice",
        ...
        try:
            ...
            return ProvisionResult(ok=False, retryable=True,
                                   error=str(e)[:500], step="mail_jobs")
        rows = (await ctx.db.execute(
            select(MailProvisioningJob.status).where(
                MailProvisioningJob.invoice_id == inv_id)
        )).scalars().all()
        if not rows:
            return ProvisionResult(ok=True, step="mail_jobs", progress=100,
                                   meta={"jobs": 0})
        if any(s == "failed" for s in rows):
            return ProvisionResult(ok=False, retryable=True,
                                   error="mail job failed", step="mail_jobs")
        if any(s in ("pending", "processing") for s in rows):
            return ProvisionResult(ok=False, retryable=True, step="mail_jobs",
                                   progress=50,
...
class HestiaProvisioner:
    ...
    async def provision(self, ctx: ProvisionContext) -> ProvisionResult:
        ...
        try:
            ...
            return ProvisionResult(ok=False, retryable=True,
                                   error=str(e)[:500], step="hestia")
        status = result.get("status")
        if status == "success":
            return ProvisionResult(ok=True, step="finalize", progress=100,
                                   meta=result.get("meta", {}))
        if status == "waiting_input":
            return ProvisionResult(
                ok=False, waiting_input=result.get("step") or "domain",
                error=result.get("message") or result.get("reason"),
                step=result.get("step"))
        if status == "skipped":
            return ProvisionResult(ok=False, waiting_input="product",
                                   error="invoice sem produto hestia",
                                   step="validate")
        return ProvisionResult(ok=False, retryable=bool(result.get("retryable")),
...
class ProjectProvisioner:
    ...
    async def provision(self, ctx: ProvisionContext) -> ProvisionResult:
        ...
        try:
            ...
            return ProvisionResult(ok=False, retryable=True,
                                   error=str(e)[:500], step="create_project")
        ...
        if status in ("aguardando_briefing", "aguardando briefing", "pending"):
            if ctx.fulfillment.project_id != project.id:
                ...
                await ctx.db.flush()
            return ProvisionResult(ok=False, waiting_input="briefing",
                                   step="briefing", progress=70,
                                   error="Projeto criado. Aguardando briefing.",
                                   meta={"project_id": project.id})
        return ProvisionResult(ok=True, step="project", progress=100,
                               meta={"project_id": project.id})
...
class ManualProvisioner:
    ...
    async def provision(self, ctx: ProvisionContext) -> ProvisionResult:
        ...
        if strategy == "guided":
            return ProvisionResult(ok=False, waiting_input="linking",
                                   step="linking", progress=20,
                                   error="Vinculação manual pendente (ex.: stack).")
        return ProvisionResult(ok=False, waiting_input="manual_review",
                               step="manual_review", progress=10,
                               error="Acompanhamento manual necessário.")


def register_all() -> None:
    for cls in (MailProvisioner, HestiaProvisioner, ProjectProvisioner,
                ManualProvisioner):
