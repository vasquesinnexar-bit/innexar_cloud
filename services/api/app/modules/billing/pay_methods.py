...
async def create_pix_charge(
    ...
    *,
    ...
    if existing and (
        not existing.expires_at
        or (_aware(existing.expires_at) or datetime.now(UTC)) > datetime.now(UTC)
    ):
        return _attempt_view(existing)
    provider = await get_payment_provider(
        db, inv.customer_id, cust.org_id, inv.currency
    )
...
async def create_boleto_charge(
    ...
    *,
    ...
    attempt = PaymentAttempt(
        ...
        meta={
            "barcode": (
                txn.get("barcode", {}).get("content")
                if isinstance(txn.get("barcode"), dict)
                else txn.get("barcode")
            ),
            "digitable_line": (
                txn.get("barcode", {}).get("content")
                if isinstance(txn.get("barcode"), dict)
