...
def _ssl_info(domain: str) -> dict:
    ...
    try:
        ...
        with (
            ...
            ctx.wrap_socket(sock, server_hostname=domain) as ss,
        ):
        ...
        out.update(
            ...
            issuer=" ".join(
                ...
                if x and x[0][0] == "organizationName"
            )
            or None,
...
class HostingServiceLayer:
    ...
    def __init__(self, db: AsyncSession, provider: DockerHostingProvider | None = None):
        self._db = db
    ...
    def infer_role(service: str | None, image: str | None) -> str:
        ...
        ) or s in ("db", "database", "postgres", "mysql", "pg", "sqld", "pgbouncer"):
            return ComponentRole.DATABASE.value
        if any(k in img for k in ("redis", "memcached", "dragonfly")) or s in (
            "redis",
            "cache",
            "valkey",
        ):
            return ComponentRole.CACHE.value
        if any(k in img for k in ("minio/minio", "minio/mc", "seaweed", "s3")) or s in (
            "minio",
            "storage",
            "s3",
            "minio-init",
        ...
        if any(k in img for k in ("rabbitmq", "nats", "kafka")) or s in (
            "queue",
            "rabbitmq",
            "nats",
            "kafka",
        ...
        if any(k in img for k in ("traefik", "nginx", "caddy", "haproxy")) or s in (
            "proxy",
            "traefik",
            "nginx",
            "lb",
        ...
        if (
            any(k in s for k in ("worker", "celery", "cron", "scheduler", "evolution"))
            or "evolution-api" in img
        ):
            return ComponentRole.WORKER.value
        if any(k in s for k in ("api", "backend", "server")) or any(
            k in img for k in ("fastapi", "uvicorn", "gunicorn")
        ):
    ...
    def group_stacks(cls, rows: list[dict]) -> list[dict]:
        ...
        for c in rows:
            ...
            project = c.get("project") or cls._label_value(
                raw_labels, "com.docker.compose.project"
            )
            cname = c.get("name", "?")
        ...
        for g in groups.values():
            domains = sorted(
                {
                    ...
                    for d in _traefik_hosts(cls._parse_ps_labels(c.get("_labels_raw")))
                }
            )
            comp_rows = []
            for c in g["containers"]:
            ...
            states = [str(c.get("status") or "").lower() for c in g["containers"]]
            running = sum(1 for s in states if s == "running")
            health = (
                "healthy"
                if running == len(states) and states
            ...
            if not g["compose_project"]:
                platform = all(
                    cls._is_platform(None, c["name"]) for c in g["containers"]
                )
            out.append(
                {
                    "key": g["key"],
                    "compose_project": g["compose_project"],
                    "name": g["name"],
                    "workdir": g["workdir"],
                    ...
                    "domains": domains,
                    "stack_type": (
                        StackType.PLATFORM_INFRA.value
                        if platform
    ...
    async def link_stack(
        ...
        *,
        ...
        if self._is_platform(compose_project, names[0] if names else ""):
            raise HostingError(
                "platform_infra", "Infra da plataforma não é vinculável a clientes"
            )
        ...
        for cname in names:
            ...
            comp = HostingComponent(
                ...
                is_public=bool(hosts),
                internal_only=not hosts,
                ports=None,
            ...
            svc = HostingService(
                ...
                stack_id=stack.id,
                org_id=org_id,
                container_name=comp.container_name,
                container_id=comp.container_id,
                project_name=compose_project,
                root_path="/app",
                path_mode="container",
                primary_domain=stack.primary_domain,
    ...
    async def link_service(
        ...
        *,
        ...
        if path_mode == "host" and not os.path.isdir(os.path.realpath(root_path)):
        ...
        svc = HostingService(
            ...
            container_id=info.get("Id"),
            project_name=(info.get("Config") or {})
            .get("Labels", {})
    ...
    async def _require_service(
        ...
        if not svc:
            raise HostingError("unauthorized_hosting_access", "Serviço não encontrado")
        return svc
    ...
    async def overview(self, svc: HostingService) -> dict:
        ...
        return {
            ...
            "last_deploy_at": (
                svc.last_deploy_at.isoformat() if svc.last_deploy_at else None
            ),
            "last_backup_at": (
                svc.last_backup_at.isoformat() if svc.last_backup_at else None
            ),
        }
        ...
        if svc.path_mode == "host" and svc.root_path:
            ...
            if os.path.isdir(git_dir):
                ...
                for args, key in (
                    ...
                    try:
                        r = _sp.run(
                            ["git", "-C", os.path.realpath(svc.root_path), *args],
                            capture_output=True,
                            text=True,
                            timeout=10,
                        )
    ...
    async def enqueue_job(
        ...
        *,
        ...
        if await self._conflict_job(service_id, job_type):
            raise HostingError(
                "job_conflict", "Já existe operação conflitante em andamento"
            )
        if idempotency_key:
            existing = (
                await self._db.execute(
    ...
    # -- backups -----------------------------------------------------------------
    async def _do_backup(self, svc: HostingService, job: HostingJob) -> str:
        ...
        self._db.add(
            HostingBackup(
                hosting_service_id=svc.id,
                backup_type="files",
                status="completed",
                size_bytes=size,
                checksum=sha.hexdigest(),
                storage_reference=dest,
    ...
    async def _do_restore(self, svc: HostingService, job: HostingJob) -> None:
        ...
        if (
            not bak
            or not bak.storage_reference
            or not os.path.isfile(bak.storage_reference)
        ):
    ...
    async def write_file(
        ...
        *,
        ...
        rev = FileRevision(
            hosting_service_id=svc.id,
            path=paths.validate(rel, for_edit=True),
