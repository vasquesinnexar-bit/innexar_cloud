...
INTERVAL_MONTHS = {"monthly": 1, "quarterly": 3, "biannual": 6, "semiannual": 6,
                   "yearly": 12, "annual": 12}
...
async def _generate_one(db: AsyncSession, contract) -> bool:
    ...
    exists = (
        await db.execute(
            select(Invoice.id).where(Invoice.period_key == period)
        )
    ).scalar_one_or_none()
    if exists:
        return False
    ...
    if not cust:
        return False
    currency = contract.currency or cust.currency or (
        "BRL" if contract.org_id == "innexar-br" else "USD")
    items = (
        await db.execute(
    ...
    prior = (
        await db.execute(
            select(Invoice.id).where(
                ...
                Invoice.status.notin_([InvoiceStatus.CANCELED.value,
    ...
    for item in items:
        ...
        if is_setup and prior:
            continue
        amount = float(item.unit_amount) if item.unit_amount is not None \
        ...
        lines.append({
            ...
            "preferred_locale": cust.locale or ("pt-BR" if contract.org_id == "innexar-br" else "en-US"),
        })
    if not lines:
        return False
    credit_used = 0.0
    ...
    if credit > 0:
        ...
        lines.append({"description": "Crédito aplicado", "quantity": 1,
                      "unit_amount": -credit_used, "line_total": -credit_used})
    ...
    inv = Invoice(
        customer_id=contract.customer_id, status=InvoiceStatus.PENDING.value,
        due_date=due, total=total, currency=currency,
        line_items={"items": lines}, period_key=period,
    )
    db.add(inv)
    await db.flush()
    await notify_customer(
        db, cust, "invoice_created", org_id=contract.org_id, id=inv.id,
        total=f"{currency} {total:.2f}", due=due.date().isoformat(),
    )
    return True
