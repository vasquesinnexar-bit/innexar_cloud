...
**Resposta padronizada:** quando aplicável, a API usa o envelope `{ "success": true, "data": {...}, "error": null }` em sucesso e `{ "success": false, "data": null, "error": "..." }` em erro. Schemas em `app/schemas/response_envelope.py`. Uso gradual em novos endpoints e refactors.
