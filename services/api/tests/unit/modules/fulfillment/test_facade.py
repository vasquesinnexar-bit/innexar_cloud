...
async def _sub_invoice(
    db: AsyncSession, cust: Customer, product: Product, plan: PricePlan
):
    sub = Subscription(
        customer_id=cust.id,
    ...
    inv = Invoice(
        ...
        currency="USD",
        line_items=[{"description": product.name, "amount": float(plan.amount)}],
    )
    db.add(inv)
    await db.flush()
    return sub, inv
...
async def test_caso1_br_purchase_creates_contract_item_fulfillment(db_session):
    ...
    with patch("app.modules.fulfillment.handlers.ProjectProvisioner.provision") as m:
        ...
        m.return_value = ProvisionResult(
            ok=False, waiting_input="briefing", step="briefing"
        )
    ...
    n_contracts = (
        (await db.execute(select(Contract).where(Contract.customer_id == cust.id)))
        .scalars()
        .all()
    )
        ...
        (
            await db.execute(
                select(ContractItem).where(
                    ContractItem.contract_id == n_contracts[0].id
                )
            )
        )
        .scalars()
...
async def test_caso3_duplicate_stripe_webhook_single_item(db_session):
    ...
    mine = [
        ...
        in {
            ...
            for c in (
                await db.execute(
                    select(Contract).where(Contract.customer_id == cust.id)
                )
            )
            .scalars()
            .all()
...
async def test_caso4_duplicate_mp_webhook_single_item(db_session):
    ...
    item = await facade.ensure_contract_item_for_purchase(
        ...
        quantity=1,
        unit_amount=25.0,
        description="Conta adicional: a@c4",
    )
    box = EmailMailbox(
        customer_id=cust.id,
        org_id="innexar",
        email_domain_id=dom.id,
        address="a@c4.teste.innexar",
        local_part="a",
    ...
    await db.flush()
    with patch(
        "app.modules.mail.provisioning.trigger_mail_provisioning_if_needed"
    ) as m:
        m.return_value = None
        out = await facade.after_payment(db, inv.id)
...
async def test_caso5_site_project_idempotent_retry(db_session):
    ...
    async def _fake_create(db2, invoice_id: int):
        ...
        existing = (
            await db2.execute(select(Project).where(Project.subscription_id == _sub.id))
        ).scalar_one_or_none()
        if existing:
            return
        db2.add(
...
async def test_caso7_hestia_retry_no_duplicate(db_session):
    ...
    async def _flaky(db2, invoice_id: int):
        ...
        return {"status": "success", "step": "finalize", "progress": 100, "meta": {}}

    with patch(
        "app.modules.billing.provisioning.run_hestia_for_invoice", side_effect=_flaky
    ):
        out = await facade.after_payment(db, inv.id)
...
async def test_caso8_mail_fulfillment_follows_job(db_session):
    ...
    item = await facade.ensure_contract_item_for_purchase(
        ...
        quantity=1,
        unit_amount=25.0,
        description="Conta adicional",
    )
    box = EmailMailbox(
        customer_id=cust.id,
        org_id="innexar",
        email_domain_id=dom.id,
        address="a@c8.teste.innexar",
        local_part="a",
        status="pending_payment",
    )
    db.add(box)
    ...
    with patch(
        ...
        assert out.status in (
            FulfillmentStatus.PROVISIONING.value,
...
async def test_caso10_retry_forbidden_without_permission(
    client: AsyncClient, billing_enabled, db_session
):
    ...
    r = await client.post(f"/api/workspace/fulfillments/{f.id}/retry", headers=_h(user))
    assert r.status_code == 403
    # com leitura, o detalhe abre normalmente
    r = await client.get(f"/api/workspace/fulfillments/{f.id}", headers=_h(user))
    assert r.status_code == 200


async def test_caso11_tenant_isolation(
    client: AsyncClient, staff_user: User, billing_enabled, db_session
):
    ...
    r = await client.get(
        f"/api/workspace/fulfillments/{f.id}?org_id=innexar", headers=_h(staff_user)
    )
    assert r.status_code == 404
    r = await client.get(
        f"/api/workspace/fulfillments/{f.id}?org_id=innexar-br", headers=_h(staff_user)
    )
    assert r.status_code == 200
