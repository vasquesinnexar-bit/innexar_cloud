...
async def _customer(db, email="f3@example.com", org="innexar"):
    c = Customer(org_id=org, name="F3", email=email, currency="BRL",
...
async def _invoice(db, customer, **kw):
    ...
    inv = Invoice(customer_id=customer.id, status="pending",
                  total=100.0, currency="BRL", **kw)
    db.add(inv)
    await db.flush()
    return inv
...
async def test_policy_resolution_order(db_session: AsyncSession):
    ...
    db_session.add(BillingPolicy(scope="org", scope_ref="innexar",
                                 grace_period_days=3, suspend_after_days=10))
...
async def test_past_due_and_reminders(db_session: AsyncSession):
    ...
    inv = await _invoice(
        db_session, c, due_date=datetime.now(UTC) - timedelta(days=2))
...
async def test_suspend_reactivate_cycle(db_session: AsyncSession):
    ...
    inv = await _invoice(
        db_session, c, due_date=datetime.now(UTC) - timedelta(days=20))
    inv.status = InvoiceStatus.PAST_DUE.value
    svc = Service(customer_id=c.id, org_id="innexar",
                  service_type="professional_email", status="active")
    db_session.add(svc)
    await db_session.flush()
    d = EmailDomain(customer_id=c.id, org_id="innexar", domain="x.com",
                    status="active", service_id=svc.id)
    db_session.add(d)
    await db_session.flush()
    db_session.add(EmailMailbox(
        customer_id=c.id, org_id="innexar", service_id=svc.id,
        email_domain_id=d.id, address="a@x.com", local_part="a",
        status="active", external_id="a@x.com"))
    ...
    class FakeMP:
        def disable_mailbox(self, address): pass
        def enable_mailbox(self, address): pass
...
async def test_contract_generation_idempotent(db_session: AsyncSession):
    ...
    p = Product(org_id="innexar", name="E-mail", slug="professional-email",
    ...
    await db_session.flush()
    ct = Contract(customer_id=c.id, org_id="innexar", status="active",
                  currency="BRL", billing_interval="monthly", billing_day=1)
    db_session.add(ct)
    await db_session.flush()
    db_session.add(ContractItem(contract_id=ct.id, product_id=p.id,
                                price_plan_id=plan.id, quantity=2, unit_amount=25.0))
    await db_session.flush()
    ...
    def fake_pix(**kw):
        ...
        return {"id": "pay123",
                "point_of_interaction": {"transaction_data": {
                    "qr_code": "000201", "qr_code_base64": "aGVsbG8="}},
                "date_of_expiration": "2026-09-15T12:00:00.000-04:00"}

    async def _noop(*a, **k):
        return None

    monkeypatch.setattr(mp_mod.MercadoPagoProvider, "create_pix",
                        lambda self, **kw: fake_pix(**kw))
    # força provider MP independente de env
    monkeypatch.setattr(
        pay_methods, "get_payment_provider", _noop_provider)
...
async def _noop_provider(db, customer_id, org_id, currency, mode="test",
