...
@pytest.mark.asyncio
async def test_stripe_out_of_order_failed_after_paid(
    ...
    def fake_construct(body, sig, secret):
        ...
        return {
            ...
            "data": {
                "object": {
                    "subscription_details": {"metadata": {"invoice_id": str(inv.id)}}
                }
            },
        }
...
async def test_stripe_refunded(db_session: AsyncSession, monkeypatch):
    ...
    rows = (
        (await db_session.execute(select(Refund).where(Refund.invoice_id == inv.id)))
        .scalars()
        .all()
    )
...
async def test_mp_expired_fails_attempt(db_session: AsyncSession, monkeypatch):
    ...
    _orig = mp_mod.MercadoPagoProvider.handle_webhook
