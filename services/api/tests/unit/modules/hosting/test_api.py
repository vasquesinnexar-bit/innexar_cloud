...
async def _mk_customer(client: AsyncClient, h: dict, email: str) -> int:
    r = await client.post("/api/workspace/customers", headers=h,
                          json={"name": "H", "email": email})
...
async def test_link_and_list(
    ...
    r = await client.post("/api/workspace/hosting/services", headers=h,
    ...
    assert r.status_code == 201, r.text
    sid = r.json()["id"]
    l = await client.get("/api/workspace/hosting/services", headers=h,
                         params={"customer_id": cid})
    assert l.status_code == 200 and len(l.json()) == 1
    ov = await client.get(f"/api/workspace/hosting/services/{sid}/overview",
                          headers=h)
    assert ov.status_code == 200
    assert ov.json()["runtime"] in ("online", "offline", "unknown", "degraded",
                                    "suspended")
...
async def test_idor(
    ...
    r = await client.post("/api/workspace/hosting/services", headers=h,
                          json={"customer_id": cid_a,
                                "container_name": "innexar-mail-portal",
                                "root_path": "/usr/share/nginx/html"})
    sid = r.json()["id"]
    # JWT do cliente B
    cu = CustomerUser(customer_id=cid_b, email="host-idor-b@teste.innexar",
