...
def _row(name, image="img", status="running", project=None, service=None):
    ...
    return {"name": name, "image": image, "status": status, "ports": "",
            "project": project, "workdir": None, "service": service,
            "_labels_raw": labels}


def test_group_by_compose_project():
    rows = [_row("heavy-clean-web", service="web", project="heavy-clean"),
            _row("heavy-clean-api", service="api", project="heavy-clean"),
            _row("heavy-clean-pg", image="postgres:16", service="db",
                 project="heavy-clean")]
...
def test_no_prefix_grouping_for_standalone():
    rows = [_row("clinica-toufic-web", service="web"),
            _row("core-pilates-app", service="app")]
    groups = L.group_stacks(rows)
    assert len(groups) == 2
    assert all(g["compose_project"] is None for g in groups)
    assert {g["key"] for g in groups} == {
        "standalone:clinica-toufic-web", "standalone:core-pilates-app"}


def test_platform_infra_by_project():
    rows = [_row("fixelo-traefik", image="traefik:v3", service="traefik",
                 project="traefik"),
            _row("observability-prometheus", image="prom/prometheus",
                 service="prometheus", project="observability"),
            _row("innexar-mailserver", image="mailserver",
                 service="mailserver", project="innexar-mail")]
...
def test_customer_project_with_same_prefix_as_infra_stays_customer():
    ...
    rows = [_row("mail-relay-cliente", image="relay", service="relay",
                 project="cliente-x")]
    groups = L.group_stacks(rows)
...
def test_label_value_survives_traefik_commas():
    raw = ("traefik.http.routers.x.rule=Host(`a.com`) || Host(`b.com`),"
...
def test_health_degraded_and_stopped():
    rows = [_row("a-web", service="web", project="p"),
            _row("a-db", image="postgres:16", service="db", project="p",
                 status="exited")]
...
def test_workspace_postgres_is_platform_infra():
    rows = [_row("innexar-usa-workspace-postgres", image="postgres:15",
                 service="postgres", project="innexar-usa-workspace")]
    g = L.group_stacks(rows)[0]
