...
export function OnboardingSection({ customerId }: { customerId: string }) {
  ...
  useEffect(() => {
    workspaceFetchStaff(apiPath(WORKSPACE_API_PATHS.ONBOARDING.CUSTOMER(customerId)))
      .then(async (res) => {
    ...
    <div className="bg-white/5 backdrop-blur border border-white/10 rounded-2xl p-6">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
        <h3 className="text-lg font-semibold text-white flex items-center gap-2">
          <ListChecks className="w-5 h-5" /> Onboardings
        </h3>
        <Link href="/onboarding" className="text-sm text-blue-300 hover:underline">
          Central →
        </Link>
      </div>
      {loading ? (
          ...
          {rows.map((r) => (
            <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 py-2 border-b border-white/5 text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <Link href={`/onboarding/${r.id}`} className="text-blue-300 hover:underline text-xs">
                  Ver detalhes
                </Link>
              </div>
            </li>
