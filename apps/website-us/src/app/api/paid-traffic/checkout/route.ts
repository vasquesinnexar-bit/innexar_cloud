...
export async function POST(request: NextRequest) {
  try {
    ...
    const success_url = `${SITE_URL.replace(/\/$/, '')}/${locale}/paid-traffic/success`
    const cancel_url = `${SITE_URL.replace(/\/$/, '')}/${locale}/paid-traffic`
    ...
    console.error('Paid traffic checkout proxy error:', err)
    return NextResponse.json(
